...
class HomeAssistantServiceTest {
    ...
    private val homeAssistantCache = mockk<HomeAssistantCache>(relaxed = true)
    private val service = HomeAssistantService(homeAssistantClient, homeAssistantCache)
    ...
    fun `listEntities should return formatted strings`() {
        ...
        every { homeAssistantCache.getEntities(null, null) } returns entities
    ...
    fun `listEntities should pass domain and area filter`() {
        ...
        every { homeAssistantCache.getEntities("light", "Kitchen") } returns entities

        val result = service.listEntities("light", "Kitchen")

        assertEquals(1, result.size)
        assertEquals("light.kitchen|Kitchen Light|Kitchen|First Floor|on||", result[0])
        verify { homeAssistantCache.getEntities("light", "Kitchen") }
    }
    
    @Test
    fun `listAreas should return list of strings`() {
        val areas = listOf("Kitchen", "Living Room")
        every { homeAssistantCache.getAreas() } returns areas
        ...
        verify { homeAssistantCache.getAreas() }
    }
    ...
    fun `getState should return entity state`() {
        ...
        every { homeAssistantCache.getEntity(entityId) } returns state
    ...
    fun `callService should delegate to client`() {
        ...
        verify { homeAssistantClient.callService("light", "turn_on", "light.kitchen", mapOf("brightness" to 100)) }
    }
}
