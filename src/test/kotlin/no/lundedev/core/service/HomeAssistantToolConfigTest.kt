...
class HomeAssistantToolConfigTest {
    ...
    fun `list_areas should call service and return string`() {
        ...
        val result = toolConfig.listAreas(homeAssistantService).apply(HomeAssistantToolConfig.EmptyRequest())
    ...
    fun `list_entities should call service with filters`() {
        ...
        val result = toolConfig.listEntities(homeAssistantService).apply(HomeAssistantToolConfig.ListEntitiesRequest("light", "Living Room"))
    ...
    fun `list_entities should return all entities if filter yields empty result`() {
        ...
        val result = toolConfig.listEntities(homeAssistantService).apply(HomeAssistantToolConfig.ListEntitiesRequest("light", "Attic"))
    ...
    fun `get_state should return entity state`() {
        ...
        val result = toolConfig.getState(homeAssistantService).apply(HomeAssistantToolConfig.GetStateRequest("light.living_room"))
    ...
    fun `call_service should execute service call`() {
        ...
        val result = toolConfig.callService(homeAssistantService).apply(HomeAssistantToolConfig.CallServiceRequest(
            domain = "light",
            service = "turn_on",
            entity_id = "light.living_room",
            payload_json = "{\"brightness\": 255}"
        ))
    ...
    fun `list_entities with no arguments should return all entities`() {
        ...
        val result = toolConfig.listEntities(homeAssistantService).apply(HomeAssistantToolConfig.ListEntitiesRequest())
